...
- Business actions: Search, Add, Edit, Delete (Draft only), Submit-for-approval, Approve, Reject, Generate QR Code/Barcode Tag (opens the Asset QR Code Tag print report for the selected asset).
...
### Plan Asset Verification List
- Description: An executive plans a verification cycle by building a list of assets to be verified and assigning that list to an employee.
- Data points (header): VERIFICATION_CYCLE_NO (auto), VERIFICATION_DATE, BRANCH_CODE, LOCATION_CODE (optional — leave blank to include all locations in the branch), CATEGORY_CODE (optional asset-category filter used when building the list), NOT_VERIFIED_SINCE_MONTHS (numeric input 'X' — include only assets not physically verified in the last X months), VERIFIER_EMPLOYEE_CODE (the employee the list is assigned to), STATUS (Planned/In Progress/Pending Approval/Closed/Cancelled).
- Data points (lines): ASSET_CODE, ASSET_NAME, EXPECTED_LOCATION_CODE, EXPECTED_EMPLOYEE_CODE, PHYSICAL_STATUS_FOUND, VERIFIED_DATE, REMARKS, ASSET_PHOTO_URL, SCAN_QR_IMAGE_URL (image picked or captured from the device holding the asset's QR/barcode tag; used only to derive the asset code, not stored as evidence).
- Business rules: the Load Assets button, on the header record, opens a pick list of capitalized assets from FAM_ASSET_REGISTER matching that header's BRANCH_CODE and (when set) LOCATION_CODE — a blank header LOCATION_CODE means "All Locations" and matches every location in the branch — optionally restricted to CATEGORY_CODE, and excludes any asset whose most recent FAM_PHYSICAL_VERIFICATION_LINE.VERIFIED_DATE falls within the last NOT_VERIFIED_SINCE_MONTHS months; assets already on the cycle are not offered again; an asset may appear only once per cycle. The user selects individual assets or all of them from the pick grid; the selection is appended as new verification lines below. Loading is only allowed while the cycle is Planned or In Progress.
- Business actions: Search, Add, Edit, Load Assets, Submit for Approval, Approve, Cancel (header); Add, Delete, Scan to Confirm, Mark Not Found, Mark Damaged (lines).
  - Scan to Confirm: the verifier may either key the ASSET_CODE directly or pick/capture the asset's QR-code image from the device into SCAN_QR_IMAGE_URL; when an image is supplied the system decodes the QR/barcode payload, normalises it to an asset code (trimming, upper-casing, and extracting the code when the payload is a URL or an 'ASSET_CODE=<code>' string), verifies the decoded asset belongs to this verification cycle, and then fills the line's ASSET_CODE, ASSET_NAME, EXPECTED_LOCATION_CODE and EXPECTED_EMPLOYEE_CODE from FAM_ASSET_REGISTER before stamping VERIFIED_DATE and PHYSICAL_STATUS_FOUND = FOUND. Manual key-in of ASSET_CODE remains fully supported — the image is optional and is only an alternative way of supplying the asset code.

### Execute Asset Verification
- Description: The employee to whom a verification list is assigned executes the physical check against that list.
- Data points: VERIFICATION_CYCLE_NO (selected from lists assigned to the user and still Planned/In Progress), SCAN_ASSET_CODE (keyed manually or sent by a wedge scanner), SCAN_ASSET_CODE_IN (hidden — the landing field the Assistant scan intent writes the camera-decoded asset code into), and per line ASSET_CODE, ASSET_NAME, PHYSICAL_STATUS_FOUND (Found/Not Found/Damaged/Extra - Unregistered), VERIFIED_DATE, REMARKS, ASSET_PHOTO_URL.
- Business rules: the scanned/entered asset id must already exist as a line of the selected verification cycle — if it does not, the entry is rejected with an error and nothing is recorded; on a successful scan the matching line is stamped Found with today's VERIFIED_DATE and the cycle moves from Planned to In Progress; the executing user may then change the physical status, add remarks and attach a photo before saving. The execution screen cannot add or delete lines — the list is fixed at planning time.
  - On the screen itself, the Scan Asset action verifies whatever asset id is currently held in SCAN_ASSET_CODE against the open cycle — the code may be keyed by hand or sent in by a hardware wedge scanner.
  - Live camera scanning is provided by the Assistant's scan intent PV_SCAN_ASSET: the verifier opens the Assistant, taps Scan, and the device camera decodes the asset QR/barcode. Because the scan carries no screen context, the intent invokes the Scan Asset By Code action, which resolves the verification cycle from the scanned asset itself rather than from the form. The intent runs deterministically (one required parameter, no confirmation step), so it works without an AI call.
  - The scanned payload is normalised before matching — control characters are stripped, a URL / ASSET_CODE=<code> / JSON wrapper is unwrapped, the asset-tag payload 'FAM|<asset code>|<category>|<branch>|<timestamp>' written by the Asset QR Tag report is reduced to the asset code alone, and the result is trimmed and upper-cased. A bare asset code passes through unchanged.
  - Scan Asset By Code accepts an asset only when it already sits on exactly one open (Planned/In Progress) verification cycle; if the asset is on no open list, or on more than one, nothing is recorded and the verifier is told to open the cycle and use Scan Asset instead. A line that already carries a finding (Not Found/Damaged/Extra - Unregistered) is reported back and left untouched. On success the line is stamped Found with today's VERIFIED_DATE, a Planned cycle moves to In Progress, and the reply states how many assets remain.
  - Manual keying of SCAN_ASSET_CODE remains fully supported — scanning is an additional capture method, not a replacement.
- Business actions: Search (own assigned lists), Scan Asset, Scan Asset By Code (invoked by the PV_SCAN_ASSET scan intent), Edit line findings, Save.
...
## Asset QR Code Tag (R)
- Layout: Printable asset tag/label (A5 landscape, 595 x 420 pt, PDF by default, HTML also available), sized so the QR code is large enough to read on screen and scan from the printed page. The report prints a BATCH - one label page per matching asset, ordered by ASSET_CODE - so a whole location can be tagged in a single run.
- Criteria: LOCATION_CODE (prompt labelled "Location Name", lookup to Location Master listing all active locations by LOCATION_NAME) is MANDATORY and has no default - it is the trigger: as soon as a Location Name is selected the report generates the list of QR label pages and shows them in the preview screen. ASSET_TYPE (prompt, fixed dropdown) is OPTIONAL and defaults to "any": the dropdown itself lists only the two real asset types, Fixed Asset and Other Asset, with no "(Any)" entry, and the criterion opens with nothing selected. Left alone it includes every asset at the chosen location regardless of its asset type, including assets whose ASSET_TYPE has not been classified; picking Fixed Asset or Other Asset narrows the batch to that type. ASSET_CODE (prompt labelled "Asset Code", lookup to the Asset Register listing assets by ASSET_NAME) is OPTIONAL and defaults to "any" (%), so the standalone Location Name flow is unchanged and never asks for it; left alone it places no restriction at all. Its purpose is to give the Asset Register row action a criterion to bind the selected asset to - a report parameter with no matching criterion binds NULL, so without this criterion the action's asset code is silently discarded and every asset at the selected location prints instead of the single one chosen.
- Content: A scannable QR code encoding the asset's QR/Barcode Tag when one is stored, otherwise the derived payload FAM|<ASSET_CODE>; the payload is also printed in plain text beneath the code. Alongside it: ASSET_CODE, ASSET_NAME, CATEGORY_NAME, BRANCH_NAME - LOCATION_NAME, SERIAL_NO and MAKE/MODEL.
- Invocation: Standalone from the Reports menu by choosing a Location Name (and optionally narrowing by Asset Type), which prints every matching asset's label; or from the Asset Register screen's row action "Generate QR/Barcode Tag", which passes the selected record's ASSET_CODE, ASSET_TYPE and LOCATION_CODE so exactly one label is produced. ASSET_CODE is the one that guarantees the single label, because it is the unique key; the other two only ever match the selected asset's own type and location and so cannot widen the run.
...
- Visuals:
  ...
  - Asset Disposal Value – Sale vs Scrap (Waterfall) — PROFIT_LOSS_ON_DISPOSAL summed by DISPOSAL_TYPE (Sale/Scrap) from the Asset Disposal transaction for the selected financial year and branch (excludes Cancelled disposals); drills through to Asset Register Report (R) filtered to disposed assets.
  - Monthly Depreciation Posting Compliance (Gauge vs 100% target) — percentage of eligible in-service assets (capitalized on/before FY end, matching branch) with current-period Companies Act depreciation posted. Note: reads 0% until the Generate Depreciation – Companies Act (Monthly) process has actually been run and its posting register is populated — the gauge honestly shows 0 rather than fabricating a number in the interim.
  - Asset Verification Coverage by Category (Pie-Chart) — slices = asset category, value = count of verified in-service assets in that category within the selected Verification Period From date; sourced from the same data as the Asset Verification Coverage by Category (V) grid.